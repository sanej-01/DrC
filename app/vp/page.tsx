...
function TrendChart({ trend }: { trend: TrendPoint[] }) {
  ...
  const lineFor = (key: 'quality' | 'bug_risk' | 'coverage') => {
    const pts = trend
      .map((p, i) => ({ v: p[key], i }))
  ...
  const series: { key: 'quality' | 'bug_risk' | 'coverage'; label: string; color: string }[] = [
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ maxHeight: 180 }}>
        ...
        {!hasAnyLine && (
          <text x={W / 2} y={H / 2} textAnchor="middle" fontSize="13" fill="var(--ink-3)">
            Not enough scored PRs yet to draw a trend
          </text>
        )}
      </svg>
      {/* legend */}
